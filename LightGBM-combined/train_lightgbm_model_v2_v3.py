...
os.makedirs('LightGBM-combined', exist_ok=True)
...
def train_lightgbm_model_with_cv(X, y, features_name, save_prefix, n_folds=5):
    """Train LightGBM model with K-Fold Cross Validation and return results"""
    print(f"\n{'='*80}")
    print(f"Training LightGBM model with {features_name}")
    ...
    # Save CV results
    cv_df.to_csv(f'LightGBM-combined/{save_prefix}_cv_results.csv', index=False)
    
    # Combine and save all fold predictions
    all_predictions = pd.concat(fold_predictions, ignore_index=True)
    all_predictions.to_csv(f'LightGBM-combined/{save_prefix}_cv_predictions.csv', index=False)
    
    # Combine and save all fold feature importance
    all_importance = pd.concat(fold_feature_importance, ignore_index=True)
    all_importance.to_csv(f'LightGBM-combined/{save_prefix}_cv_feature_importance.csv', index=False)
    ...
    model_final.fit(
        ...
        y_train_full,
        eval_set=[(X_test_scaled_final, y_test_full)],
        eval_metric='rmse',
        callbacks=[
            lgb.early_stopping(stopping_rounds=10),
    ...
    plt.tight_layout()
    plt.savefig(f'LightGBM-combined/{save_prefix}_final_feature_importance.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'LightGBM-combined/{save_prefix}_final_actual_vs_predicted.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    # Save final model
    model_final.booster_.save_model(f'LightGBM-combined/{save_prefix}_final_model.txt')
    ...
    predictions_final_df.to_csv(f'LightGBM-combined/{save_prefix}_final_predictions.csv', index=False)
...
results_with_leakage = train_lightgbm_model_with_cv(
    X_with_leakage, y, 
    "Features WITH Leakage (includes CumulativeGasUsed)", 
    "lightgbm_with_leakage"
)
...
results_without_leakage = train_lightgbm_model_with_cv(
    X_without_leakage, y, 
    "Features WITHOUT Leakage (excludes CumulativeGasUsed)", 
    "lightgbm_without_leakage"
)
...
plt.tight_layout()
plt.savefig('LightGBM-combined/comprehensive_analysis.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig('LightGBM-combined/leakage_comparison_analysis.png', dpi=300, bbox_inches='tight')
plt.close()
...
comparison_df.to_csv('LightGBM-combined/model_comparison_summary.csv', index=False)

print("\nAll results saved to LightGBM-combined/ directory:")
...
print(f"   - LightGBM WITHOUT leakage achieves {results_without_leakage['r2']:.1%} R² accuracy")
...
print("\n" + "="*80)
print("LIGHTGBM COMBINED V2+V3 ANALYSIS COMPLETE!")
print("="*80)
