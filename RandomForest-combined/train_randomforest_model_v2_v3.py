...
os.makedirs('RandomForest-combined', exist_ok=True)
...
def train_randomforest_model_with_cv(X, y, features_name, save_prefix, n_folds=5):
    """Train Random Forest model with K-Fold Cross Validation and return results"""
    print(f"\n{'='*80}")
    print(f"Training Random Forest model with {features_name}")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_train_full), 1):
        ...
        # Set Random Forest parameters optimized for large dataset
    ...
    # Save CV results
    cv_df.to_csv(f'RandomForest-combined/{save_prefix}_cv_results.csv', index=False)
    
    # Combine and save all fold predictions
    all_predictions = pd.concat(fold_predictions, ignore_index=True)
    all_predictions.to_csv(f'RandomForest-combined/{save_prefix}_cv_predictions.csv', index=False)
    
    # Combine and save all fold feature importance
    all_importance = pd.concat(fold_feature_importance, ignore_index=True)
    all_importance.to_csv(f'RandomForest-combined/{save_prefix}_cv_feature_importance.csv', index=False)
    
    # Now train final model on full training set with default parameters
    print(f"\nTraining final model with optimized default parameters...")
    ...
    # Use optimized default parameters (no grid search needed)
    best_params = {
        'n_estimators': 100,
        'max_depth': 15,
        'min_samples_split': 5,
    ...
    plt.tight_layout()
    plt.savefig(f'RandomForest-combined/{save_prefix}_final_feature_importance.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'RandomForest-combined/{save_prefix}_final_feature_importance_horizontal.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'RandomForest-combined/{save_prefix}_final_actual_vs_predicted.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'RandomForest-combined/{save_prefix}_final_residuals.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    # Save final model (using joblib for sklearn models)
    import joblib
    joblib.dump(model_final, f'RandomForest-combined/{save_prefix}_final_model.joblib')
    ...
    predictions_final_df.to_csv(f'RandomForest-combined/{save_prefix}_final_predictions.csv', index=False)
...
results_with_leakage = train_randomforest_model_with_cv(
    X_with_leakage, y, 
    "Features WITH Leakage (includes CumulativeGasUsed)", 
    "randomforest_with_leakage"
)
...
results_without_leakage = train_randomforest_model_with_cv(
    X_without_leakage, y, 
    "Features WITHOUT Leakage (excludes CumulativeGasUsed)", 
    "randomforest_without_leakage"
)
...
plt.tight_layout()
plt.savefig('RandomForest-combined/comprehensive_analysis.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig('RandomForest-combined/leakage_comparison_analysis.png', dpi=300, bbox_inches='tight')
plt.close()
...
comparison_df.to_csv('RandomForest-combined/model_comparison_summary.csv', index=False)

# Calculate execution time
execution_time = time.time() - start_time

print("\nAll results saved to RandomForest-combined/ directory:")
...
print(f"   - Random Forest WITHOUT leakage achieves {results_without_leakage['r2']:.1%} R² accuracy")
...
print("\n" + "="*80)
print("RANDOM FOREST COMBINED V2+V3 ANALYSIS COMPLETE!")
print("="*80)
